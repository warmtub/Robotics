...
int __cdecl main(void) 
{
	...
	float K = 1./(xcal1*ycal2-xcal2*ycal1);
	cout << "K =" << K << endl;
	...
	th[0] = Objectstacked_Centroid_and_Angle[0][2];
	X[0] = K*(x[0]*(Xcal1*ycal2-Xcal2*ycal1)+y[0]*(-Xcal1*xcal2+Xcal2*xcal1));
	Y[0] = K*(x[0]*(Ycal1*ycal2-Ycal2*ycal1)+y[0]*(-Ycal1*xcal2+Ycal2*xcal1));
	...
	for (int i = 1; i < Objectstacked_Centroid_and_Angle.size(); ++i)
	{
		...
		th[i] = Objectstacked_Centroid_and_Angle[i][2];
		X[i] = K*(x[i]*(Xcal1*ycal2-Xcal2*ycal1)+y[i]*(-Xcal1*xcal2+Xcal2*xcal1));
		Y[i] = K*(x[i]*(Ycal1*ycal2-Ycal2*ycal1)+y[i]*(-Ycal1*xcal2+Ycal2*xcal1));
		
		char AboveObject[100];
		sprintf(AboveObject, "MOVL %f %f %f %f 0 180", X[i], Y[i], Zcal1+Hblock*i, 90-th[i]);
		char OnObject[100];
		sprintf(OnObject, "MOVL %f %f %f %f 0 180", X[i], Y[i], Zcal1, 90-th[i]);
		char AboveStackObject[100];
		sprintf(AboveStackObject, "MOVL %f %f %f %f 0 180", X[0], Y[0], Zcal1+Hblock*(i+1), 90-th[0]);
		char OnStackObject[100];
		sprintf(OnStackObject, "MOVL %f %f %f %f 0 180", X[0], Y[0], Zcal1+Hblock*i, 90-th[0]);
		cout << AboveObject << endl;
		cout << OnObject << endl;
		cout << AboveStackObject << endl;
		cout << OnStackObject << endl;
